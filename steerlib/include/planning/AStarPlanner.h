...
namespace SteerLib
{
	...
	/*
		...
		@operators 
		The greater than, less than and equals operator have been overloaded. This means that objects of this class 
		can be used with these operators. Change the functionality of the operators depending upon your implementation
	...
	*/
	class STEERLIB_API AStarPlannerNode{
		public:
			...
			Util::Point point;
			int index;
			AStarPlannerNode(Util::Point _point, double _g, double _f, int _index)
			{
				f = _f;
				point = _point;
				g = _g;
	...
	class STEERLIB_API AStarPlanner{
		public:
			...
			*/
			bool canBeTraversed ( int id, double OBSTACLE_CLEARANCE  );
			/*
			...
			*/
			...
			bool computePath(std::vector<Util::Point>& agent_path, Util::Point start, Util::Point goal, SteerLib::GridDatabase2D * _gSpatialDatabase, bool append_to_path, double OBSTACLE_CLEARANCE);
			double calculateHvalueManhattan(Util::Point point1, Util::Point point2);
			double calculateHvalueEuclidean(Util::Point point1, Util::Point point2);

			int findlowestF(std::vector<AStarPlannerNode> List);
			bool isnodeinlist(std::vector<AStarPlannerNode> List, Util::Point point);
			int findnodeinopenlist(std::vector<AStarPlannerNode> List, Util::Point point);
			void printlistcontent(std::vector<AStarPlannerNode> List);


			void construct_path(std::vector<Util::Point>& agent_path, Util::Point start, std::vector<AStarPlannerNode> List, AStarPlannerNode node);
		private:
